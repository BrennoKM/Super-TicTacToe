...
- **Revisão (paleta global):** cada quadro tinha sua própria paleta de 64 cores (`quantize` por quadro), então o GIF saía com uma tabela de cores global (a do primeiro quadro, exigida pelo formato) e uma tabela local extra em cada quadro seguinte, um GIF tecnicamente válido, mas fora do caso mais comum (uma paleta global só). Relato do usuário: o GIF que antes enviava pelo WhatsApp parou de enviar depois dessa demanda. Não deu pra reproduzir a falha específica do WhatsApp (o arquivo antigo já validava certo em `identify`/inspeção manual de cabeçalho, contudo), mas a paleta por quadro é a diferença técnica mais plausível e concreta em relação à versão anterior (mais simples), então virou paleta única combinando os pixels de todos os quadros (`generateGif`, duas passadas: desenha e guarda cada quadro, depois indexa todos pela mesma paleta). Sai ~5% mais leve de quebra. Ainda precisa de confirmação em teste real no WhatsApp.
